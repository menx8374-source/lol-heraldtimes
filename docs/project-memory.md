...
## 記事フォーマットの方針変更（ユーザー決定 2026-07-25・全10スプリント完了後）
- **参考サイト（おばにゅー等）準拠の「まとめ速報レス形式」に変更**: 記事本文を「AI要約段落＋短い引用」から、**掲示板/SNSの書き込みを番号付きレスとして羅列**する形式（`番号: 名前`＋本文複数行＋重要行の赤/オレンジ強調＋`>>N`アンカー）に作り替える。名前は匿名化ハンドル（国内プレイヤーさん／海外プレイヤーさん等）。
- **レス本文は逐語表示（ユーザーがリスク承知で選択）**: 参考サイト同様、収集した書き込みをそのまま表示する転載モデル。これに伴い **F9 の「逐語コピー回避（逐語一致率）チェックは無効化/緩和**する（逐語が意図的なため）。**ただし NGワード・個人中傷/晒し・出典欠落・未確認ラベルの安全フィルタは維持**（生の掲示板書き込みは差別語・中傷が混ざりやすく、外すと危険）。
- **受容したリスク（明示的に確認済み）**: 掲示板/SNSの逐語転載は著作権・各サービスの転載規約（5ch等）・**AdSenseの複製コンテンツポリシー**（無価値な複製サイトは収益化不可・アカウント停止リスク）に触れうる。恒久運営に向けては、実収集接続時に転載許諾の確認・出典明記の徹底・オプトアウト対応（F15で実装済み）を運用で担保する必要がある。
- 出典リンク・AI自動生成注記・Riot非公認ディスクレーマー・引用/転載である旨の表示は維持する（F15）。
