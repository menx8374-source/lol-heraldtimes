...
- **未適用パッチpreview記事の確定チェック実行（パッチ記事刷新S5、opt-in）**: `npm run confirm-patch-preview`（`PATCH_PREVIEW_MODE=on`のときのみ動作。本番反映後(Post側でversions.jsonの現行live版と一致)にpreview記事(速報バッジ付き)を確定版へin-place更新し速報バッジを外す。`update-articles`と同じ運用パターンでcron等から定期実行する想定。offでは即座にno-op）
- **PBE記事の生成/更新チェック実行（PBE-S4、opt-in）**: `npm run pbe-article`（`PBE_ARTICLE_MODE=on`のときのみ動作。CommunityDragon（キー不要・無料）でpbe版≠latest版（本番準備中）を確認できた場合のみ、チャンピオン基本ステータス/cost/cooldown・アイテムの数値diffから未確定バッジ付きのPBE先行記事を生成/上書き更新する（`externalId="pbe-<バージョン>"`で一意化・重複記事なし）。スキル効果量・X収集は含まない。AI不使用。offでは即座にdisabledでDB・外部APIに一切アクセスしない）
- **統合パイプライン実行（F10・F11）**: `npm run pipeline`（収集→重複排除→記事生成→タイトル生成→安全フィルタ→公開までを1回の起動で人手介入なしで実行する。`npm run collect`+`npm run generate` を1本のオーケストレーションにまとめたもの）
  ...
  - 各実行の収集件数・記事化候補数・生成成功/失敗数・公開数・保留数・要レビュー数は `PipelineRunLog` テーブルに記録される（運営ダッシュボードが参照する）。
- **公開ポリシー・レビューキュー（運営CMS v2 admincms-S1）**: `/admin` に「公開ポリシー」パネルと「レビューキュー（要レビュー）」セクションが追加された。
  - カテゴリ（5chの反応／海外の反応／Xの反応／パッチ/メタ／Riot公式／eスポーツ）ごとに「自動公開」か「要レビュー」かを切り替えられる（`CategoryPublishPolicy` テーブルに永続化・アプリ再起動後も保持）。**既定は全カテゴリ「要レビュー」**。
  - 自動収集パイプラインは記事のカテゴリのポリシーを参照し、安全フィルタ通過後、自動公開カテゴリなら従来どおり `status="published"`、要レビューカテゴリなら `status="review"` で保存する（公開サイトには一切表示されない）。安全フィルタ不通過（NGワード等）は、カテゴリが自動公開でも従来どおり `status="held"`（保留キュー）になる（held優先）。
  - `status="review"` の記事は `/admin` のレビューキューから「プレビュー」（`/admin/articles/[id]/preview`、公開ページと同一体裁・Basic認証内・robots除外・状態を変更しない）・「編集」・「承認して公開」・「却下」を行える。承認すると `status="published"` になり公開サイトの一覧に即座に反映される（一覧再検証 `revalidatePublishedListings` を呼ぶ）。却下すると `status="rejected"` になる。
  - 既定「全カテゴリ要レビュー」のため、本追加を有効化した直後は運営者が承認しない限り新規記事が公開されない。`/admin` 冒頭の「未レビュー N件」バッジで気付けるようにしている。
...
| `PATCH_PREVIEW_MODE` | 任意 | 未適用パッチの先行速報＋本番反映後の自動確定（パッチ記事刷新S5、opt-in）。既定`off`（未設定含む）では収集・記事化・表示すべて現状と完全同一（回帰ゼロ）。`on`: 現行live版(versions.json先頭)の次パッチの公式ノートが既に公開されていれば、速報バッジ付き(「【速報・未適用】...」の段落＋タイトル接頭辞「【速報】」)で先行記事化する(AI不使用・公式ノート逐語＋注意書きのみ)。本番反映後は`npm run confirm-patch-preview`で同一記事(`externalId`=パッチ番号で一意特定)を確定版へ自動更新する(重複記事は作らない) |
| `PBE_ARTICLE_MODE` | 任意 | PBE（テストサーバー）記事枠（PBE-S4、opt-in）。既定`off`（未設定含む）では収集・記事化・表示すべて現状と完全同一（回帰ゼロ・追加コストなし）。`on`: CommunityDragon（キー不要・無料）でpbe版≠latest版を確認できた場合のみ、`npm run pbe-article`実行時にチャンピオン基本/cost/cooldown・アイテムの数値diffから未確定バッジ付きのPBE先行記事を生成/上書き更新する（`externalId="pbe-<バージョン>"`で一意化）。スキル効果量・X収集は含まない |
...
| `PUBLISH_SCHEDULE_MODE` | 任意 | 投稿スケジュール分散（成長G6）。`immediate`（既定）: 現状どおり即時公開。`schedule`: 反応記事(reddit/5ch)を公開時刻テーブル(07:15/12:15/15:15/18:15/21:15 JST)に分散予約する（`status="scheduled"`＋`scheduledAt`割当、到来時に自動昇格）。免除ソース(riot/riot-news)は速報性維持のため常に即時公開 |
| `DISCORD_WEBHOOK_URL` | 任意 | 配信導線 — Discord Webhook通知（成長G6）。**秘密情報のため必ず `.env` のみに設定しコミットしない**。設定時のみ、公開された記事のタイトル＋URLをこのWebhookへ通知する。未設定時は完全no-op |
